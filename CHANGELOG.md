...
Откат 6.4.0: контент-план замедлил агента, а не ускорил.

#### Изменено

- **Пайплайн вернулся** к `0 Готовность → 1 Вход и инвентарь → 2 Площадка → 3 Три страницы →
  4 Сдача`: тексты снова сочиняются по ходу сборки страниц. Обе сборки откачены тем же
  коммитом.

#### Почему

- Замер владельца на одном и том же компоненте, агент Figma: сборка с контент-планом —
  **19 минут**, прежним пайплайном — **16 минут**. План не убрал ни одной операции на холсте,
  но добавил фазу генерации всего текста вперёд — для агента это чистый налог ~20 % времени.
  Цифры записаны здесь, чтобы идея «сначала весь контент, потом заливка» не вернулась без
  нового аргумента: возвращать её имеет смысл только точечно и только если просядет качество
  формулировок, а не скорость.

### 6.4.0 — 30.08.2026

Контент отделён от механики: сначала весь текст, потом тупая заливка.

#### Изменено

- **Пайплайн перестроен:** `0 Готовность → 1 Чтение → 2 Контент-план → 3 Площадка →
  4 Заливка → 5 Сдача`. Чтение — один проход по компоненту сверху вниз, в порядке панели
  свойств. Контент-план сочиняет каждую строку трёх страниц до первого касания холста;
  у каждой строки пометка источника — из компонента, из шаблона или пробел.
- **Порог стал двусторонним:** до закрытия плана в файл не пишется ничего, после закрытия
  не сочиняется ничего. Строка, которой не хватило при заливке, — ошибка плана: вернуться
  и дописать план, а не импровизировать на холсте.
- Причина: тексты сочинялись в момент сборки, внимание делилось между формулировками и
  слотами — страдали оба. У агента Figma это заметнее всего.
- Сборка для агента перестроена тем же коммитом — правило сопровождения.

#### Отклонено

- Интервью перед сборкой — рассматривалось как способ наполнить смысловые блоки
  (зачем конфигурация, чем стили отличаются) и отклонено владельцем: вопросы убивают
  поток, а пробелы остаются пробелами по правилу «пустой блок лучше выдумки».

### 6.3.2 — 29.08.2026

#### Изменено

- Сборка для агента Figma описывает экосистему в явном виде: кит — бесплатный файл
  Community и живёт сам по себе, скилл автоматизирует сборку прямо внутри Figma, полная
  версия для Claude Code — в репозитории, рядом парный `spec-kit-theme`. Куски используются
  по отдельности. Архив Claude Code по содержанию не менялся.
...
- Сборка для агента Figma описывает экосистему: коллекция `theme` приезжает с бесплатным
  файлом Community, темы можно собирать и руками, скилл это автоматизирует прямо внутри
  Figma; полная версия для Claude Code — в репозитории, рядом парный `spec-kit-docs`.
  Архив Claude Code по содержанию не менялся.
