...
- **Шаг 0.2 — проверка библиотеки.** Движок ищется как подключённая библиотека `Component Spec Kit`; при её отсутствии скилл работает по локальной копии с предупреждением либо останавливается с просьбой подключить. Подключить библиотеку из кода нельзя.
