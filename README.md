...
- **Figma-файл движка:** [Оформление документации по компонентам](https://www.figma.com/design/JQ10AuRUEFp0KGoOyT3iHJ/)
...
Скилл ищет компоненты движка **по имени**, а не по id, поэтому работает в любой копии файла. Имена движка — часть контракта: переименовывать `ds-*` нельзя, включая опечатки в них (`ds-doc/interation`, `ds-paragraph-ux-wtiting`).

### Как устроена секция

| Страница | Паттерн | Про что |
|---|---|---|
| История изменений | `ds-doc/changelog` | версия, тип изменения, дата, авторы |
| Спецификация | `ds-doc/specification` | лид, анатомия, блок на каждое свойство |
| Анимация | `ds-doc/interation` | триггер → реакция, motion-токены |
| Советы и практики | `ds-doc/tips-practices` | пары «делать / не делать» |
| Микрокопирайт | `ds-doc/microcopy` | правила текста по слотам |
| Компоненты | `ds-doc/components` | сам component set с подписями осей |
...
```bash
git clone <ссылка на репозиторий> ~/src/component-documentation
