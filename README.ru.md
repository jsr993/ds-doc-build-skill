...
> Скилл один на оба языка: **язык документации следует за языком запроса**. Написали по-русски — содержание блоков будет русским, по-английски — английским. Заголовки разделов при этом приходят из переменных Figma-файла и следуют его языку.
...
1. Скачайте архив. В `dist/` лежит одна и та же сборка в двух формах — выбирайте по способу установки:
   ...
   У каждой формы есть двойник с версией в имени (`ds-doc-build-4.1.0.skill`, `ds-doc-build-4.1.0.zip`), чтобы сборка на диске опознавалась не открываясь. **В `dist/` лежит только текущая версия** — прошлые сборка удаляет, за историей в git.
2. **Сначала удалите старую папку** — не распаковывайте поверх. Состав референсов между поколениями меняется, и оставшийся файл заставит новый `SKILL.md` считать папку долитой:
...
3. Распакуйте архив в папку скиллов:
   ...
   Путь важен: Claude Code показывает в списке скиллов только те папки, где **прямо внутри лежит `SKILL.md`** с frontmatter. Пустая папка или файлы уровнем глубже — скилл в списке не появится, и никакой ошибки при этом не будет.
...
Дальше вмешиваться не нужно вообще. Скилл отчитается инвентарём — сколько вариантов, какие оси, в каком порядке пойдут блоки спецификации — и соберёт три страницы, показывая снимок после каждой, до конца.
...
│   ├── ds-doc-build-4.0.0.skill  # он же, версия в имени
│   ├── ds-doc-build.zip          # обёрнут в папку ds-doc-build/
│   └── ds-doc-build-4.0.0.zip    # он же, версия в имени
├── scripts/
