...
**Работает автономно.** От ссылки до готовой секции — без плана на подтверждение и без интервью. Это сделано под поточную задачу: задокументировать подряд десятки компонентов, а не один. Вопрос возникает ровно там, где скилл останавливается: нет библиотеки, на входе не компонент, не нашлась переменная движка — и один раз перед переносом вашего компонента в слот на странице `Components`.
...
1. Скачайте архив. В `dist/` лежит одна и та же сборка в двух формах — выбирайте по способу установки:
   ...
   У каждой формы есть двойник с версией в имени (`ds-doc-build-4.0.0.skill`, `ds-doc-build-4.0.0.zip`), чтобы сборка на диске опознавалась не открываясь. Имена без версии — постоянные адреса, в них всегда текущая версия.
...
Дальше вмешиваться почти не нужно. Скилл отчитается инвентарём — сколько вариантов, какие оси, в каком порядке пойдут блоки спецификации — и соберёт три страницы, показывая снимок после каждой. Единственный вопрос за всю сборку — разрешение перенести ваш component set внутрь секции документации.

Можно вызвать скилл и напрямую: `/ds-doc-build`.

### Где он всё-таки остановится

Остановок ровно пять, и все — про то, что дальше идти бессмысленно или нельзя без вас:
...
- перенос вашего component set в `Slot Component` — единственное действие за пределами секции документации, без разрешения не делается.
...
| ось в столбцы на `Components` | первая VARIANT-ось в порядке объявления свойств |
...
- Не меняет исходный компонент — свойства, слои, имя и описание только читает. Перенос сета в `Slot Component` — только с вашего разрешения.
