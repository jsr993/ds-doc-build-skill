...
Страницы файла на 04.08.2026: `component kit` (движок: секция `pattern` — шаблоны страниц, секция `components` — атомы с подсекциями `paragraph`, `component`, `logs`) и `information` (руководство внутри файла: фреймы `Information` и `Claude Code`).

**Имена страниц и секций — справочные, не контрактные.** Скилл ищет компоненты по имени обходом всего документа (`findAllWithCriteria`), а не по расположению, поэтому перестановка и переименование страниц сборку не ломают. Контракт — только имена самих компонентов `ds-*` и их property. Список выше нужен, чтобы человек понимал, где что лежит, и не более того.
...
Проверять до всего остального: `figma.teamLibrary.getAvailableLibraryVariableCollectionsAsync()` вернёт коллекции подключённых библиотек с полями `name` и `libraryName`. Есть коллекция `decoration` — работаем, а `libraryName` называем в отчёте, чтобы человек видел источник. Таких библиотек несколько — спросить, какую брать. Нет ни одной, но в файле лежат локальные `ds-*` и коллекция `decoration` — это дубликат шаблона, работать можно, но предупредить. Ни того ни другого — остановиться и попросить человека подключить библиотеку через `Assets` → `Libraries`.
