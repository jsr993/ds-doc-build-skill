...
Гейт — не вопрос пользователю. Отчитался фактами и пошёл дальше. Остановка только там, где прямо написано «стоп»: не читаются референсы, нет библиотеки, на входе не компонент, не найден `ds-*` или переменная.
